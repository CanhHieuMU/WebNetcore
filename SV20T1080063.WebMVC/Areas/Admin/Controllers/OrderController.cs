...
namespace SV20T1080063.WebMVC.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult Create()
        {
            ...
            var nguDan = ApplicationContext.GetSessionData<PaginationSearchInput>(ORDER_SEARCH);

            return View(inputFromSession);
        }
        /// Thầy tách minh họa thôi, chứ còn lại tự lo mà làm.
        ...
        {
            int length = 5;
            var inputFromSession = ApplicationContext.GetSessionData<PaginationSearchInput>(CART);
            ...
            // Luu lai vao session dieu kien Tim kiem
            ApplicationContext.SetSessionData(CART, input);
            var nguDan = ApplicationContext.GetSessionData<PaginationSearchInput>(CART);
        ...
        public IActionResult Delete(int id = 0)
        {
            ...
            var orderData = OrderDataService.GetByID(id);
            if (orderData != null)
            {
                if (orderData.Status == 1 || orderData.Status == 2 || orderData.Status == 3 || orderData.Status == 4)
                {
                    TempData["ErrorMessage"] = "Không thể xóa đơn hàng này";
                    return RedirectToAction("Details", new { id = id });
                }
            }
            bool success = OrderDataService.Delete(id);

            if (!success)
            {
                TempData["ErrorMessage"] = "Xóa đơn hàng có lỗi";
                return RedirectToAction("Details", new { id = id });
            }
