...
namespace SV20T1080063.WebMVC.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult Photo(int id = 0, string method = "add", int photoId = 0)
        {
            switch (method)
            {
                case "add":
                    ...
                    return View(model);
                case "edit":
                    var pp = ProductDataService.GetPhoto(photoId);
                    return View(pp);
                case "delete":
                    //TODO: Delete photo
                    var del = ProductDataService.DeletePhoto(photoId);
        ...
        public IActionResult AddPhoto(ProductPhoto data, IFormFile? uploadPhoto)
        {
            ...
            var pro = ProductDataService.GetProduct(data.ProductId);

            if (!ModelState.IsValid)
                return View("Create", pro);

            var photoId = ProductDataService.AddPhoto(data);

            if (photoId > 0)
            {
                return RedirectToAction("Edit", pro);
            }
